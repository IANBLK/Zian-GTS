#!/usr/bin/env python3
"""Read-only Zian GTS WAL inspection. Never repairs, truncates or replays a journal."""
import argparse
import hashlib
import json
import pathlib
...
def records(path):
    ...
    with path.open("rb") as source:
        while True:
            ...
            sequence += 1
            if event["version"] != 1 or event["sequence"] != sequence:
...
def main():
    ...
    parser.add_argument("--export", type=pathlib.Path, help="New directory for complete JSON records, including SNBT checkpoints")
